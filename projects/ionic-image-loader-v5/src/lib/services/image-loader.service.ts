...
export class ImageLoaderService {
  ...
  async clearImageCache(imageUrl: string) {
    ...
    this.runLocked(async () => {
      ...
      try {
        let file = await route.getFile(fileName);
        await file.delete();

        if (this.isWKWebView && !this.isIonicWKWebView) {
          file = await FileSystemPlugin.getSystemDirectory(SystemDirectoryType.Temp).getFile(fileName);
          await file.delete();
        }
      } catch (err) {
        this.throwError(err);
  ...
  private async indexCache(): Promise<void> {
    ...
    try {
      const directory = await this.getFileCacheDirectory().getDirectory(this.config.cacheDirectoryName);
   ...
   */
  private async removeFile(fileName: string): Promise<any> {
    let directory = await this.getFileCacheDirectory().getDirectory(this.config.cacheDirectoryName);
    let file = await directory.getFile(fileName);
    await file.delete();

    if (this.isWKWebView && !this.isIonicWKWebView) {
      try {
        directory = await FileSystemPlugin.getSystemDirectory(SystemDirectoryType.Temp).getDirectory(this.config.cacheDirectoryName)
        file = await directory.getFile(fileName);
        await file.delete();
      } catch (err) {
  ...
  private async getCachedImagePath(url: string): Promise<string> {
    ...
    // get full path
    const dir = await this.getFileCacheDirectory().getDirectory(this.config.cacheDirectoryName);
    const tempDir = await FileSystemPlugin.getSystemDirectory(SystemDirectoryType.Temp).getDirectory(this.config.cacheDirectoryName);
    ...
    try {
      ...
      try {
        ...
        // copy it!
        const tempFileEntry = await tempDir.getFile(fileName);
        const file = await dir.getFile(fileName);
        await file.copyTo(tempFileEntry);
   ...
   */
  private async directoryExistsOrThrow(directory: Promise<Directory>): Promise<Directory> {
    return directory.then(directory =>
      directory.exists().then((exists) => {
        if (!exists) throw new Error("Directory not exists");
        return directory;
      })
    )
  }
  ...
  private createCacheDirectory(replace: boolean = false): Promise<any> {
    ...
    if (replace) {
      ...
      cacheDirectoryPromise = cacheDir.then((cacheDir) => cacheDir.create({replace: replace}));
    } else {
      // check if the cache directory exists.
      // if it does not exist create it!
      cacheDirectoryPromise = this.directoryExistsOrThrow(cacheDir)
        .catch((cacheDir) => cacheDir.create({replace: false}));
    }
    ...
    if (this.isWKWebView && !this.isIonicWKWebView) {
      ...
      if (replace) {
        ...
        tempDirectoryPromise = tempDir.then((tempDir) => tempDir.create({replace: replace}));
      } else {
        // check if the temp directory exists.
        // if it does not exist create it!
        tempDirectoryPromise = this.directoryExistsOrThrow(tempDir)
          .catch((tempDir) => tempDir.create({replace: false}));
      }
    } else {
